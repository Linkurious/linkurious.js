...
/*
ICI LE PREVIOUS NODE RENDERRER AVEC IMAGE


+ TIMEOUT QUI FONCTIONNE
...
nodes_def.previous = function(node, context, settings) {
    var fontSize,
...
;(function() {
   ...
   */
  node_node_def.no_draw_image = function(node, context, settings, options) {
    var o = options || {},
        prefix = settings('prefix') || '',
        size = node[prefix + 'size'] || 1,
        x = node[prefix + 'x'],
...
;(function() {
   ...
   * @param  {?object}                  options  Force optional parameters (e.g. color).
   */
  node_node_def.pixel_snapping = function(node, context, settings, options) {
    var o = options || {},
        prefix = settings('prefix') || '',
        size = node[prefix + 'size'] || 1,
        x = node[prefix + 'x'],
